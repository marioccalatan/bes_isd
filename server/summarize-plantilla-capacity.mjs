...
await withLocalConnection(async (connection) => {
  const result = await connection.execute(`
    ...
    LEFT JOIN (
      SELECT p.*, COALESCE(p.dept_id, od.dept_id) resolved_dept_id
      FROM bes_positions p
      LEFT JOIN bes_offices o ON o.office_id=p.office_id
      LEFT JOIN bes_departments od ON od.department_id=o.department_id
    ) p ON p.resolved_dept_id=d.dept_id
