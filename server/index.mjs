    ...
    if (req.method === 'POST' && req.url === '/api/member-programs/csr') {
      ...
      await withConnection(async (c) => {
        ...
        await c.execute(`INSERT INTO bes_csr_requests (csr_uid,date_requested,program_type,requestee,designation,organization,registration_details,sector,location,barangay,municipality,district,project_details,project_requirement,pending_reason,with_letter_reply,institutional,additional_remarks,request_status,approval_status,evaluation_result,evaluated_by,date_approved,date_released,amount_funding,pjrs,actual_project_cost,created_by_user_id,updated_by_user_id)
          VALUES (:csrRequestUid,TO_DATE(:csrDateRequested,'YYYY-MM-DD'),:csrProgramType,:csrRequestee,:csrDesignation,:csrOrganization,:csrRegistrationDetails,:csrSector,:csrLocation,:csrBarangay,:csrMunicipality,:csrDistrict,:csrProjectDetails,:csrProjectRequirement,:csrPendingReason,:csrWithLetterReply,:csrInstitutional,:csrAdditionalRemarks,:csrRequestStatus,:csrApprovalStatus,:csrEvaluationResult,:csrEvaluatedBy,TO_DATE(:csrDateApproved,'YYYY-MM-DD'),TO_DATE(:csrDateReleased,'YYYY-MM-DD'),:csrAmountFunding,:csrPjrs,:csrActualProjectCost,:csrActorUserId,:csrActorUserId)`, {
