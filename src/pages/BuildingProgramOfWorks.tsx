...
export default function BuildingProgramOfWorks() {
  ...
  const label = mode === 'Daily' ? `${format(cells[0].start, 'MMM d')} – ${format(cells.at(-1)!.end, 'MMM d, yyyy')}` : mode === 'Weekly' ? 'Eight-week plan' : mode === 'Monthly' ? format(anchor, 'MMMM yyyy') : mode === 'Quarterly' ? `Q${Math.floor(anchor.getMonth() / 3) + 1} ${format(anchor, 'yyyy')}` : format(anchor, 'yyyy');
