...
export default function BuildingProgramOfWorks() {
  ...
  const [subFacilityFilter, setSubFacilityFilter] = useState('');
  ...
  const isWithinFacility = (id: string, ancestorId: string) => {
    const visited = new Set<string>();
    let current = facilityById.get(id);
  ...
  const rootFacilities = (data?.facilities ?? []).filter((facility) => !facility.parentId || !facilityById.has(facility.parentId)).sort((a, b) => a.sortOrder - b.sortOrder || a.name.localeCompare(b.name));
  const subFacilities = facilityFilter ? (data?.facilities ?? []).filter((facility) => facility.parentId === facilityFilter).sort((a, b) => a.name.localeCompare(b.name)) : [];
  const selectedFacility = subFacilityFilter || facilityFilter;
  const filteredProjects = (data?.projects ?? []).filter((project) => !selectedFacility || isWithinFacility(project.facilityId, selectedFacility));
  const reportControls = data ? <BfmProjectReportControls facilities={data.facilities} projects={filteredProjects} includeDateFilter /> : null;
  ...
  const visibleRows = rows.filter((project) => (!selectedFacility || isWithinFacility(project.facilityId, selectedFacility)) && rangeStart && rangeEnd && !isAfter(parseISO(project.startDate || project.targetDate), rangeEnd) && !isBefore(parseISO(project.targetDate), rangeStart));
  const completedCount = visibleRows.filter((project) => project.status === 'Completed').length;
  const accomplishedPercent = visibleRows.length ? (completedCount / visibleRows.length) * 100 : 0;
