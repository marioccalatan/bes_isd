...
export function MemberProgramsCsr({ onCountChange, onAddToPrograms, programType, title = 'Corporate Social Responsibility', description = 'CSR requests, policy evaluation, project requirements, and funding.', requestLabel = 'CSR Request' }: MemberProgramsCsrProps) {
  ...
  const [reportPrintOpen, setReportPrintOpen] = useState(false); const [reportFormat, setReportFormat] = useState<CsrReportFormat>('date'); const [reportDateFrom, setReportDateFrom] = useState(''); const [reportDateTo, setReportDateTo] = useState(''); const [fitReport, setFitReport] = useState(false);
  ...
  <Dialog open={reportPrintOpen} onClose={() => setReportPrintOpen(false)} title="Print CSR Report" description="Choose how the printed report should be arranged." size="sm" footer={<div className="flex w-full justify-between gap-2"><Button variant="ghost" onClick={() => { setReportDateFrom(''); setReportDateTo(''); }}>Clear Dates</Button><div className="flex gap-2"><Button variant="outline" onClick={() => setReportPrintOpen(false)}>Cancel</Button><Button onClick={() => { setReportPrintOpen(false); printReport(reportFormat, reportDateFrom, reportDateTo, fitReport); }}><Printer className="h-4 w-4" /> Print</Button></div></div>}><div className="space-y-4"><Field label="Report Format"><Select value={reportFormat} onChange={(event) => setReportFormat(event.target.value as CsrReportFormat)}><option value="date">Sorted by Date</option><option value="standard">Standard</option><option value="district">Grouped by District</option><option value="municipality">Grouped by Municipality</option><option value="programType">Grouped by Program Type</option><option value="evaluationType">Grouped by Evaluation Type</option></Select></Field><label className="flex cursor-pointer items-center gap-2 text-sm font-medium text-slate-700"><Checkbox checked={fitReport} onChange={(event) => setFitReport(event.target.checked)} />Fit report</label><div className="grid gap-3 sm:grid-cols-2"><Field label="Date From"><Input type="date" value={reportDateFrom} max={reportDateTo || undefined} onChange={(event) => setReportDateFrom(event.target.value)} /></Field><Field label="Date To"><Input type="date" value={reportDateTo} min={reportDateFrom || undefined} onChange={(event) => setReportDateTo(event.target.value)} /></Field></div>{!reportDateFrom && !reportDateTo && <p className="text-xs text-slate-500">All time date will be printed.</p>}</div></Dialog>
