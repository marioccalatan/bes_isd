...
type CsrReportFormat = 'date' | 'district' | 'municipality' | 'programType' | 'evaluationType' | 'standard';
type CsrReportPeriod = 'thisYear' | 'lastYear' | 'thisMonth' | 'lastMonth' | 'custom';
...
const CSR_REPORT_PERIOD_LABELS: Record<CsrReportPeriod, string> = {
  thisYear: 'This Year',
  lastYear: 'Last Year',
...
function reportPeriodRange(period: Exclude<CsrReportPeriod, 'custom'>, baseDate = new Date()) {
  ...
  if (period === 'thisMonth') return { from: dateInputValue(new Date(year, month, 1)), to: dateInputValue(new Date(year, month + 1, 0)) };
  return { from: dateInputValue(new Date(year, month - 1, 1)), to: dateInputValue(new Date(year, month, 0)) };
}
...
export function MemberProgramsCsr({ onCountChange, onAddToPrograms, programType, title = 'Corporate Social Responsibility', description = 'CSR requests, policy evaluation, project requirements, and funding.', requestLabel = 'CSR Request' }: MemberProgramsCsrProps) {
  ...
  const initialReportRange = useMemo(() => reportPeriodRange('thisYear'), []);
  const [requests, setRequests] = useState<CsrRequest[]>([]); const [locations, setLocations] = useState<BarangayLocation[]>([]); const [sectors, setSectors] = useState<string[]>([]); const [budgetAllocations, setBudgetAllocations] = useState<CsrBudgetAllocation[]>([]); const [search, setSearch] = useState('');
  ...
  const [reportPrintOpen, setReportPrintOpen] = useState(false); const [reportFormat, setReportFormat] = useState<CsrReportFormat>('date'); const [reportPeriod, setReportPeriod] = useState<CsrReportPeriod>('thisYear'); const [reportDateFrom, setReportDateFrom] = useState(initialReportRange.from); const [reportDateTo, setReportDateTo] = useState(initialReportRange.to); const [fitReport, setFitReport] = useState(false);
  ...
  const [hoveredCsr, setHoveredCsr] = useState<CsrRequest | null>(null); const [showDetailsOnHover, setShowDetailsOnHover] = useState(false); const [csrContextMenu, setCsrContextMenu] = useState<{ item: CsrRequest; x: number; y: number } | null>(null);
  async function load() { if (!token) return; try { const [csrRequests, barangayLocations, csrSectors, csrBudgets] = await Promise.all([fetchCsrRequests(token), fetchBarangayLocations(token), fetchCsrSectors(token), fetchCsrBudgetAllocations(token)]); const scopedRequests = programType === COMMUNITY_RELATIONS_PROGRAM_TYPE ? csrRequests.filter((item) => COMMUNITY_RELATIONS_PROGRAM_TYPES.includes(item.programType)) : programType ? csrRequests.filter((item) => item.programType === programType) : csrRequests.filter((item) => !COMMUNITY_RELATIONS_PROGRAM_TYPES.includes(item.programType)); setRequests(scopedRequests); onCountChange?.(scopedRequests.length); setLocations(barangayLocations); setSectors(csrSectors); setBudgetAllocations(csrBudgets); } catch (error) { toast({ kind: 'error', title: `Unable to load ${requestLabel.toLowerCase()} records`, description: error instanceof Error ? error.message : 'Please try again.' }); } }
  ...
  function csrRowClassName(item: CsrRequest) {
    if (item.status === 'Completed') return 'bg-blue-50/70 hover:bg-blue-100/70';
    if (item.status === 'Pending') return 'bg-gold-50/80 hover:bg-gold-100/70';
    return undefined;
  }
  function updateReportPeriod(period: CsrReportPeriod) {
  ...
  function edit(item: CsrRequest) { setEditing(item); const { id: _id, updatedAt: _updatedAt, ...values } = item; const evaluated = item.status === 'Completed' && item.evaluationResult.length > 0; setForm({ ...emptyForm, ...values, evaluationResult: evaluated ? item.evaluationResult : [], approvalStatus: evaluated ? item.approvalStatus : 'For Evaluation', dateApproved: evaluated ? item.dateApproved : '', dateReleased: values.dateReleased || '', closedApproved: evaluated && item.approvalStatus === 'Approved' ? Boolean(values.closedApproved) : false }); setPendingFiles([]); setAttachments([]); if (token) void fetchCsrAttachments(token, item.id).then(setAttachments).catch((error) => toast({ kind: 'error', title: 'Attachments not loaded', description: error instanceof Error ? error.message : 'Please try again.' })); setOpen(true); }
  ...
  function reportGroupValue(item: CsrRequest, format: CsrReportFormat) {
    if (format === 'district' || format === 'standard') return item.institutional ? 'INSTITUTIONAL' : item.district || 'Unspecified District';
  ...
  function reportBudgetYears(items: CsrRequest[], dateFrom = '', dateTo = '') {
    if (dateFrom || dateTo) {
      const startYear = Number((dateFrom || dateTo).slice(0, 4)) || new Date().getFullYear();
      const endYear = Number((dateTo || dateFrom).slice(0, 4)) || startYear;
      const first = Math.min(startYear, endYear);
      const last = Math.max(startYear, endYear);
      return Array.from({ length: last - first + 1 }, (_, index) => first + index);
    }
    const years = new Set<number>();
    items.forEach((item) => { const year = Number(item.dateRequested?.slice(0, 4)); if (Number.isInteger(year)) years.add(year); });
    budgetAllocations.forEach((allocation) => years.add(allocation.year));
    return years.size ? [...years] : [new Date().getFullYear()];
  }
  function reportBudgetFor(district: string, programType: string, years: number[]) {
    const targetDistrict = district.trim().toLowerCase();
    const targetProgramType = programType.trim().toLowerCase();
    return budgetAllocations.reduce((sum, allocation) => years.includes(allocation.year) && allocation.district.trim().toLowerCase() === targetDistrict && allocation.programType.trim().toLowerCase() === targetProgramType ? sum + (Number(allocation.budget) || 0) : sum, 0);
  }
  ...
  function sortedReportRows(format: CsrReportFormat, dateFrom = '', dateTo = '') {
    const programTypeRank = new Map(PROGRAM_TYPES.map((value, index) => [value.toLowerCase(), index]));
    return rows.filter((item) => (!dateFrom || item.dateRequested >= dateFrom) && (!dateTo || item.dateRequested <= dateTo)).sort((a, b) => {
      if (format === 'standard') {
        const districtComparison = reportGroupValue(a, format).localeCompare(reportGroupValue(b, format), undefined, { numeric: true, sensitivity: 'base' });
        if (districtComparison !== 0) return districtComparison;
        const leftRank = programTypeRank.get((a.programType || '').toLowerCase()) ?? Number.MAX_SAFE_INTEGER;
        const rightRank = programTypeRank.get((b.programType || '').toLowerCase()) ?? Number.MAX_SAFE_INTEGER;
        if (leftRank !== rightRank) return leftRank - rightRank;
        const programComparison = String(a.programType || '').localeCompare(String(b.programType || ''), undefined, { sensitivity: 'base' });
  ...
  function reportTable(items: CsrRequest[], format: CsrReportFormat = 'date', dateFrom = '', dateTo = '', fit = false) {
    if (format === 'standard') return standardReportTable(items, dateFrom, dateTo, fit);
  ...
  function standardReportTable(items: CsrRequest[], dateFrom = '', dateTo = '', fit = false) {
    ...
    const body = districts.map((district) => {
      ...
      const rows = programTypes.map((programType) => {
        const programItems = districtItems.filter((item) => item.programType === programType);
        const budget = reportBudgetFor(district, programType, years);
        if (fit && !budget && !programItems.length) return '';
        const span = Math.max(programItems.length, 1);
        const detailRows = programItems.length ? programItems : [null];
        return detailRows.map((item, rowIndex) => {
          const evaluationStatus = item?.status === 'Pending' && item.pendingReason ? `${item.status} - ${item.pendingReason}` : item?.status;
          const values = item ? [++itemNumber,item.dateRequested,item.requestee,item.organization,item.sector,item.municipality,item.barangay,item.district,evaluationStatus,item.evaluationResult.length ? item.evaluationResult.join(', ') : 'Not Evaluated',item.approvalStatus,item.withLetterReply ? 'Yes' : 'No',item.dateApproved,item.dateReleased,item.amountFunding ? Number(item.amountFunding).toLocaleString('en-PH',{ style:'currency',currency:'PHP' }) : '',item.actualProjectCost ? Number(item.actualProjectCost).toLocaleString('en-PH',{ style:'currency',currency:'PHP' }) : '',item.additionalRemarks] : ['—','','','','','','','','','','','','','','','',''];
          const programCells = rowIndex === 0 ? `<td rowspan="${span}">${escapeHtml(programType)}</td><td rowspan="${span}">${budget ? escapeHtml(budget.toLocaleString('en-PH',{ style:'currency',currency:'PHP' })) : '—'}</td>` : '';
          return `<tr><td>${escapeHtml(String(values[0] || '—'))}</td>${programCells}${values.slice(1).map((value) => `<td>${escapeHtml(String(value || '—'))}</td>`).join('')}</tr>`;
        }).join('');
      }).join('');
      return `<tr class="group-row"><td colspan="${headers.length}">District: ${escapeHtml(districtSummary)}</td></tr>${rows}`;
    }).join('');
    return `<table><thead><tr>${headers.map((header) => `<th>${header}</th>`).join('')}</tr></thead><tbody>${body}</tbody></table>`;
  }
  ...
  const columns: Column<CsrRequest>[] = [
    ...
    { key: 'dateReleased', header: 'Date Released', sortable: true, filterable: true, render: (item) => item.dateReleased || '—' },
  ...
  return <><Card>
    ...
    <CardContent>
      ...
      <DataTable columns={columns} rows={pagedRows} getRowId={(item) => item.id} onRowClick={edit} onRowMouseEnter={(item) => showDetailsOnHover && setHoveredCsr(item)} onRowMouseLeave={() => setHoveredCsr(null)} onRowContextMenu={openCsrContextMenu} cardTitle={(item) => item.programType} rowClassName={csrRowClassName} sortKey={sortKey} sortDir={sortDir} onSort={sortBy} columnFilters={columnFilters} onColumnFilterChange={(key, value) => setColumnFilters((current) => ({ ...current, [key]: value }))} minWidthPx={2180} emptyTitle={programType ? 'No requests' : 'No CSR requests'} emptyDescription={programType ? 'Add the first Linkages request.' : 'Add the first Corporate Social Responsibility request.'} />
...
function csrDetailFields(item: CsrRequest): Array<[string, string]> {
  ...
  return [
    ...
    ['Implemented', item.closedApproved ? 'Yes' : 'No'],
    ['Date Approved/Disapproved', item.dateApproved],
    ['Date Released', item.dateReleased],
