...
import { BarChart3, BookOpen, Check, ChevronLeft, ChevronRight, File, Plus, Printer, Upload, X } from 'lucide-react';
...
import { addCsrProjectEvent, createCsrSector, deleteCsrAttachment, deleteCsrRequest, downloadCsrAttachment, fetchBarangayLocations, fetchCsrAttachments, fetchCsrProjectEvents, fetchCsrRequests, fetchCsrSectors, fetchUserDirectory, saveCsrRequest, uploadCsrAttachment, type BarangayLocation, type CsrAttachment, type CsrProjectEvent, type CsrRequest, type DirectoryUser, type MemberProgramInput } from '@/lib/api';
...
export function MemberProgramsCsr({ onCountChange, onAddToPrograms, programType, title = 'Corporate Social Responsibility', description = 'CSR requests, policy evaluation, project requirements, and funding.', requestLabel = 'CSR Request' }: MemberProgramsCsrProps) {
  ...
  const columns: Column<CsrRequest>[] = [
    ...
    { key: 'withLetterReply', header: 'Letter Reply', className: 'text-center', sortable: true, render: (item) => item.withLetterReply ? <Check className="mx-auto h-5 w-5 text-emerald-500" aria-label="With letter reply" /> : '—' },
