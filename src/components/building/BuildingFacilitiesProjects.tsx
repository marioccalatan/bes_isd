...
import { Building2, CalendarClock, CalendarRange, ChevronDown, ChevronRight, MapPin, Paperclip, Pencil, Plus } from 'lucide-react';
...
import { BfmProjectDocumentsDialog } from '@/components/building/BfmProjectDocumentsDialog';
import { Input, Label, Select, Textarea } from '@/components/ui/input';
import { useAuth } from '@/context/AuthContext';
import { useToast } from '@/context/ToastContext';
import { createBfmProjectFacility, createBfmProject, deleteBfmProjectFacility, deleteBfmProject, fetchBfmProjects, updateBfmProjectFacility, updateBfmProject, type BfmFacility, type BfmOperationsData, type BfmProject } from '@/lib/api';
...
export function BuildingFacilitiesProjects() {
  ...
  const [documentsProject, setDocumentsProject] = useState<BfmProject | null>(null);
  const [projectForm, setProjectForm] = useState(emptyProject);
  const [saving, setSaving] = useState(false);

  useEffect(() => { fetchBfmProjects(token).then((next) => { setData(next); setExpanded(new Set(next.facilities.filter((item) => !item.parentId).map((item) => item.id))); }).catch((error) => toast({ kind: 'error', title: 'Unable to load projects', description: error instanceof Error ? error.message : 'Please try again.' })); }, [token, toast]);
  ...
  async function saveFacility() {
    ...
    try {
      const next = editingFacility
        ? await updateBfmProjectFacility(token, editingFacility.id, facilityForm)
        : await createBfmProjectFacility(token, { ...facilityForm, parentId: facilityParent?.id });
      replace(next);
  ...
  async function removeFacility() {
    ...
    setSaving(true);
    try { replace(await deleteBfmProjectFacility(token, editingFacility.id)); closeFacilityDialog(); toast({ kind: 'success', title: 'Facility deleted', description: `${editingFacility.name} and its child project records were deleted from Oracle.` }); }
    ...
    return (
      <div key={facility.id} className={depth ? 'ml-4 border-l border-slate-200 pl-3' : ''}>
        ...
        {open && <div className="space-y-2">
          {projectItems.map((project) => <div key={project.id} className="ml-4 flex flex-col gap-2 rounded-lg border border-slate-200 bg-slate-50/40 p-3 sm:flex-row sm:items-start sm:justify-between"><span><span className="font-medium">{project.title}</span><span className="mt-1 flex flex-wrap gap-1"><Badge>{project.status}</Badge><Badge>{project.priority}</Badge><Badge>{project.category}</Badge><Badge>{project.budgetStatus}</Badge></span>{project.budgetAmount != null && <span className="mt-1 block text-xs font-medium text-slate-600">{new Intl.NumberFormat('en-PH', { style: 'currency', currency: 'PHP' }).format(project.budgetAmount)}</span>}{project.description && <span className="mt-1 block text-xs text-slate-500">{project.description}</span>}</span><span className="flex shrink-0 flex-wrap items-center justify-end gap-2"><span className="text-xs text-slate-500"><CalendarClock className="mr-1 inline h-3.5 w-3.5" />Target {formatDate(project.targetDate)}</span><Button size="sm" variant="outline" onClick={() => setDocumentsProject(project)}><Paperclip className="h-3.5 w-3.5" /> Related Documents</Button>{data?.canManage && <Button size="sm" variant="ghost" onClick={() => openProject(facility, project)}><Pencil className="h-3.5 w-3.5" /> Edit</Button>}</span></div>)}
  ...
  return <><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><p className="font-medium">Facility hierarchy and project work</p><p className="text-sm text-slate-500">Expand any facility to view nested areas and non-routine projects with target dates.</p></div><div className="flex flex-wrap gap-2"><BfmProjectReportControls facilities={data.facilities} projects={data.projects} /><Button variant="outline" onClick={() => window.open('/workspace/building-facilities/program-of-works', '_blank', 'noopener,noreferrer')}><CalendarRange className="h-4 w-4" /> Program of Works</Button>{data.canManage && <Button onClick={() => openAddFacility(null)}><Plus className="h-4 w-4" /> Add Facility</Button>}</div></div><div className="space-y-3">{(children.get('ROOT') ?? []).map((facility) => renderFacility(facility))}</div>
  ...
  <BfmProjectDocumentsDialog project={documentsProject} open={Boolean(documentsProject)} onClose={() => setDocumentsProject(null)} />
  </>;
}
