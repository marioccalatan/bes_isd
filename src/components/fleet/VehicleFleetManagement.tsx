...
export function VehicleFleetManagement() {
  ...
  return <div className="space-y-5">
    ...
    <Dialog open={Boolean(editingModelLibraryId)} onClose={() => { if (!savingModelLibrary && !modelPreviewOpen) setEditingModelLibraryId(null); }} title="Edit Vehicle Model" description={`Update ${vehicleModels.find((item) => item.id === editingModelLibraryId)?.brand ?? ''} ${vehicleModels.find((item) => item.id === editingModelLibraryId)?.model ?? ''}.`} size="md" footer={<div className="flex w-full items-center justify-between gap-2"><Button variant="destructive" disabled={savingModelLibrary} onClick={() => setModelLibraryDeleteOpen(true)}><Trash2 className="h-4 w-4" /> Delete</Button><div className="flex flex-wrap gap-2"><Button variant="outline" disabled={savingModelLibrary || (!editingModelLibraryFile && !vehicleModels.find((item) => item.id === editingModelLibraryId)?.model3d)} onClick={() => setModelPreviewOpen(true)}>Preview</Button><Button variant="outline" disabled={savingModelLibrary} onClick={() => setEditingModelLibraryId(null)}>Cancel</Button><Button disabled={savingModelLibrary || !editingModelLibraryForm.brand.trim() || !editingModelLibraryForm.model.trim()} onClick={() => void saveEditedModelLibraryItem()}>{savingModelLibrary ? 'Saving…' : 'Save Changes'}</Button></div></div>}>
    ...
    {modelPreviewOpen && editingModelLibraryId && <VehicleModelPreview token={token} modelId={editingModelLibraryId} name={`${editingModelLibraryForm.brand} ${editingModelLibraryForm.model}`} file={editingModelLibraryFile} onClose={() => setModelPreviewOpen(false)} />}
