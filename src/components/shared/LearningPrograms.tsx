...
export function LearningPrograms({ onCountChange }: { onCountChange: (count: number) => void }) {
  ...
  function openForm(row?: TrainingSeminar) {
    setEditingId(row?.id);
    setForm(row ? { name: row.name, address: row.address ?? '', dateFrom: row.dateFrom ?? '', dateTo: row.dateTo ?? '', hours: row.hours == null ? '' : String(row.hours), programCost: row.programCost == null ? '' : row.programCost.toFixed(2), budgetCost: row.budgetCost == null ? '' : row.budgetCost.toFixed(2), type: row.type ?? '', conductedBy: row.conductedBy ?? '', status: row.status ?? '', workplan: row.workplan ?? '', categories: row.categories ?? [] } : emptyForm);
    setFormError('');
    setOpen(true);
  }
  ...
  async function submit() {
    ...
    if (programCost !== null && (!Number.isFinite(programCost) || programCost < 0 || programCost > 9999999999.99 || Math.abs(programCost * 100 - Math.round(programCost * 100)) > 0.0001)) { setFormError('Actual Cost Incurred must be a non-negative number with at most two decimal places.'); return; }
    const budgetCost = form.budgetCost === '' ? null : Number(form.budgetCost);
    if (budgetCost !== null && (!Number.isFinite(budgetCost) || budgetCost < 0 || budgetCost > 9999999999.99 || Math.abs(budgetCost * 100 - Math.round(budgetCost * 100)) > 0.0001)) { setFormError('Budget Cost must be a non-negative number with at most two decimal places.'); return; }
    setSaving(true);
    try {
      const saved = await saveTrainingSeminar(token, { ...form, name: form.name.trim(), hours, programCost, budgetCost, dateFrom: form.dateFrom || null, dateTo: form.dateTo || null }, editingId);
  ...
  useEffect(() => {
    ...
    fetchTrainingSeminars(token).then(({ programs: rows, canEdit: editable, categoryOptions: options }) => {
      if (!cancelled) { setPrograms(rows); setCanEdit(editable); setCategoryOptions(options); onCountChange(rows.length); }
  ...
  return <Card>
    ...
    <Dialog open={open && !manageCategories} onClose={() => { if (!saving) setOpen(false); }} title={editingId ? 'Edit Training / Seminar' : 'Add Training / Seminar'} size="lg" footer={<><Button variant="outline" disabled={saving} onClick={() => setOpen(false)}>Cancel</Button><Button type="submit" form="training-program-form" disabled={saving}>{saving ? 'Saving…' : editingId ? 'Save Changes' : 'Add Program'}</Button></>}>
      <form id="training-program-form" className="grid gap-4 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); void submit(); }}>
        ...
        <div><Label htmlFor="training-budget-cost">Budget Cost</Label><Input id="training-budget-cost" type="number" min="0" max="9999999999.99" step="0.01" placeholder="0.00" disabled={saving} value={form.budgetCost} onChange={(event) => setForm({ ...form, budgetCost: event.target.value })} onBlur={() => { if (form.budgetCost && Number.isFinite(Number(form.budgetCost)) && /^\d+(\.\d{0,2})?$/.test(form.budgetCost)) setForm((current) => ({ ...current, budgetCost: Number(current.budgetCost).toFixed(2) })); }} /></div>
        <div><Label htmlFor="training-cost">Actual Cost Incurred</Label><Input id="training-cost" type="number" min="0" max="9999999999.99" step="0.01" placeholder="0.00" disabled={saving} value={form.programCost} onChange={(event) => setForm({ ...form, programCost: event.target.value })} onBlur={() => { if (form.programCost && Number.isFinite(Number(form.programCost)) && /^\d+(\.\d{0,2})?$/.test(form.programCost)) setForm((current) => ({ ...current, programCost: Number(current.programCost).toFixed(2) })); }} /></div>
        ...
        <div className="sm:col-span-2">
          <Button type="button" size="sm" variant="outline" className="mb-2 float-right" disabled={saving} onClick={() => setManageCategories(true)}>Manage Categories</Button><span id="training-category-label" className="mb-1 block text-sm font-medium text-slate-700">Category</span>
          <details className="rounded-md border border-slate-300 bg-surface">
            ...
            <div role="group" aria-labelledby="training-category-label" className="max-h-52 space-y-1 overflow-y-auto border-t border-slate-200 p-2">
              {categoryOptions.map((category) => <label key={category.id} className="flex cursor-pointer items-start gap-2 rounded-md px-2 py-2 text-sm text-slate-700 hover:bg-slate-50"><Checkbox disabled={saving} className="mt-0.5 shrink-0" checked={form.categories.includes(category.id)} onChange={(event) => { const checked = event.target.checked; setForm((current) => ({ ...current, categories: checked ? [...current.categories, category.id] : current.categories.filter((value) => value !== category.id) })); }} /><span>{category.name}</span></label>)}
            </div>
          </details>
